  ...
  const handleClose = () => {
      ...
      const fetchUserId = async () => {
        try {
          ...
          if (parentId === null || parentId === undefined) {
            ...
            fetchContents(folderid); // Pass userId directly
          }
        } catch (err) {
    ...
    return (
      <Box sx={{ minHeight: "100vh" }} onContextMenu={handleContextMenu}>
        ...
        <Box display="flex" alignItems="center" mb={2} pl={3}>
          <Button onClick={() => {
            setFolderPath([]);
            setParentId(folderId); // root folder
            fetchContents(folderId);
          }}>
            Home
          </Button>
          {folderPath.map((folder, index) => (
            <Box key={folder.id} display="flex" alignItems="center">
              <ArrowRightIcon />
              <Button onClick={() => {
